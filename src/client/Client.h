...
protected:
  ...
  void fill_statx(Inode *in, unsigned int mask, struct statx *stx);
  void fill_statx(InodeRef& in, unsigned int mask, struct statx *stx) {
...
private:
  ...
  int _do_setattr(Inode *in, struct statx *stx, int mask, int uid, int gid, InodeRef *inp);
  void stat_to_statx(struct stat *st, struct statx *stx);
  int __setattrx(Inode *in, struct statx *stx, int mask, int uid=-1, int gid=-1, InodeRef *inp = 0);
  int _setattrx(InodeRef &in, struct statx *stx, int mask);
  int _setattr(InodeRef &in, struct stat *attr, int mask);
  int _ll_setattrx(Inode *in, struct statx *stx, int mask, int uid = -1,
  ...
  int may_setattr(Inode *in, struct statx *stx, int mask, int uid=-1, int gid=-1);
...
public:
  ...
  int statx(const char *path, unsigned int flags, unsigned int want, struct statx *stx);
  ...
  int setattrx(const char *relpath, struct statx *stx, int mask);
  ...
  int ll_getattrx(Inode *in, unsigned int flags, unsigned int want,
		  struct statx *st, int uid = -1, int gid = -1);
  int ll_setattrx(Inode *in, struct statx *stx, int mask, int uid = -1,
		 int gid = -1);
