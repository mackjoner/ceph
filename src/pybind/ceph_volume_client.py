...
def _str_to_bytes(string):
    string.encode() if not isinstance(string, bytes) else string
...
class CephFSVolumeClient(object):
    ...
    def create_volume(self, volume_path, size=None, data_isolated=False):
        ...
        if size is not None:
            self.fs.setxattr(path, 'ceph.quota.max_bytes', _str_to_bytes(str(size)), 0)
