...
TEST(LibCephFS, Btime) {
  ...
  struct statx	stx;
  ASSERT_EQ(ceph_statx(cmount, filename, 0, STATX_BASIC_STATS | STATX_BTIME, &stx), 0);
  ASSERT_TRUE(stx.stx_mask & (STATX_CTIME|STATX_BTIME));
...
TEST(LibCephFS, SetBtime) {
  ...
  ASSERT_EQ(ceph_setattrx(cmount, filename, &stx, CEPH_SETATTR_BTIME), 0);

  ASSERT_EQ(ceph_statx(cmount, filename, 0, STATX_BTIME, &stx), 0);
  ASSERT_TRUE(stx.stx_mask & STATX_BTIME);
...
TEST(LibCephFS, LazyStatx) {
  ...
  struct statx	stx;
  ASSERT_EQ(ceph_ll_getattrx(cmount2, file2, AT_NO_ATTR_SYNC, STATX_CTIME, &stx, getuid(), getgid()), 0);
  ASSERT_TRUE(stx.stx_mask & STATX_CTIME);
...
TEST(LibCephFS, ChangeAttr) {
  ...
  struct statx	stx;
  ASSERT_EQ(ceph_statx(cmount, filename, 0, STATX_BASIC_STATS | STATX_VERSION, &stx), 0);
